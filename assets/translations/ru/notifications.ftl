ntf-user-welcome =
    ...
    Спасибо, что выбрал наш VPN-бот 🔐

    Мы делаем его для тех, кому нужен нормальный, стабильный и свободный интернет без лишней головной боли: подключился — и пользуешься 🚀

    Сервис растёт именно благодаря людям, которые остаются с нами, пользуются VPN каждый день и советуют его дальше.

    Чем больше нас становится, тем больше у нас возможностей улучшать скорость, добавлять новые серверы и делать подключение ещё стабильнее 🌍

    Это простая история: ты пользуешься удобным VPN, а мы со своей стороны постоянно докручиваем сервис, чтобы он работал лучше ⚙️

    Если у тебя есть друзья, коллеги или близкие, которым тоже важен свободный доступ к интернету, приватность и стабильное подключение — просто перешли им этот бот 🤝

    Пусть нормальным интернетом пользуются не только избранные ✨
...
ntf-user =
    ...
    .email-linked = ✅ <i>Email привязан. Подписка синхронизирована с ботом и сайтом.</i>
    .email-already-used = ❌ <i>Этот email уже привязан к другому аккаунту.</i>
    .email-code-sent = 📩 <i>Код подтверждения отправлен на вашу почту.</i>
    .email-code-invalid = ❌ <i>Неверный код подтверждения.</i>
    .email-code-expired = ⌛ <i>Срок действия кода истек. Запросите новый код.</i>
    .email-code-rate-limit = ⏱ <i>Подождите немного перед повторной отправкой кода.</i>
    .email-delivery-disabled = ⚠️ <i>Отправка писем не настроена. Обратитесь в поддержку.</i>
