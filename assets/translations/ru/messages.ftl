...
msg-menu-connect-guide =
    ...
    Установите приложение <b>Happ (Proxy Utility)</b> из магазина и импортируйте ссылку подписки.

    Ваша ссылка подписки:
    <code>{ $happ_link }</code>

    Нажмите на ссылку — приложение откроется и подключится автоматически.

    После подключения проверьте, что интернет работает стабильно.
