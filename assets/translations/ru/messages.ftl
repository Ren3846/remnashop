...
msg-menu-connect-guide =
    ...
    1. Установите приложение <b>Happ (Proxy Utility)</b> с помощью кнопок ниже.

    2. Нажмите <b>«📋 Получить ссылку подписки»</b> — бот пришлёт ссылку отдельным сообщением.

    3. Нажмите на ссылку — приложение откроется и подключится автоматически.

    4. Проверьте, что интернет работает стабильно.
